...
//  mesh->get(P0, "pres2"); // Pascals
  ...
  output.write("Successfully created Laplacian solver!\n");
  ...
  if(!restarting) {
    ...
    else
    {
      ...
      output.write("Inverting ubyn!\n");
      lap->setCoefC(logn0);
      phi = lap->solve(ubyn);
      output.write("Invert complete\n");
    }
  }
  ...
  if (compress0 && (diffusion_par > 0.0) && (gamma_i_BC > 0.0) && (gamma_e_BC > 0.0) )
    {
      ...
      Jpar_sh0 *= c_se0 -  vth_e0/(2.0*sqrt(PI)) * exp( - ee*(phi0*Va*Lbar*B0*Bbar)/(KB*Te0*Tebar*eV_K) );
    }
  ...
  if (BScurrent)
    {
      ...
      nu_estar = 100.*nu_e * q95*Lbar / (vth_e) / Aratio^(1.5);
      nu_istar = 100.*nu_i * q95*Lbar / (vth_i) / Aratio^(1.5);
...
int physics_run(BoutReal t)
{
  ...
  if (compress0 && (diffusion_par > 0.0) && (gamma_i_BC > 0.0) && (gamma_e_BC > 0.0) )
    {
	  ...
	  //Jpar_sh *= c_se*Va - vth_et/(2.0*sqrt(PI)) * exp( - ee*(phi*Va*Lbar*B0*Bbar)/(KB*Te_tmp*Tebar*eV_K) );
      Jpar_sh *= c_set -  vth_et/(2.0*sqrt(PI)) * exp( - ee*((phi+phi0)*Va*Lbar*B0*Bbar)/(KB*Te_tmp*Tebar*eV_K) );
      Jpar_sh -= Jpar_sh0;
      ...
      else
	{
	  ...
	  Jpar_sh *= c_se*Va - vth_et/(2.0*sqrt(PI)) * exp( - ee*(phi*Va*Lbar*B0*Bbar)/(KB*Te0*Tebar*eV_K) );
	  Jpar_sh *= MU0*Lbar/(B0*Bbar);
	  }*/

	  q_se = -gamma_e_BC * Pe * c_se / kappa_par_e * (Nbar*density*KB);
	  q_si = -gamma_i_BC * Pi * c_se / kappa_par_i * (Nbar*density*KB); 
	  /*q_se = gamma_e_BC * ( (Pe0+Pe) * c_set - Pe0 * c_se0 ) * const_cse/Va;
      q_si = gamma_i_BC * ( (Pi0+Pi) * c_set - Pi0 * c_se0 ) * const_cse/Va;
      q_se *= - (Nbar*density*KB) / kappa_par_e;
      q_si *= - (Nbar*density*KB) / kappa_par_i;
	  */
      //sbc--xia
  ...
  if (!emass)
	...
	ddt(Ajpar) = -Grad_parP(B0*phi, CELL_CENTRE) / B0 - eta*Jpar;
      ...
      if(diamag){
	ddt(Ajpar) -= bracket(B0*phi0, Psi, bm_exb);   // Equilibrium flow
      }
